...
const restaurentsTheme = responsiveFontSizes(
	createTheme({
		breakpoints: {
			values: {
				xs: 0,
	...
	return {
		...
		MuiAlert: {
			styleOverrides: {
				standardError: {
					background: `#FFC3C3`,
					color: restaurentsTheme.palette.error.main,
				},
			},
		},
		MuiDivider: {
		...
		MuiCheckbox: {
			styleOverrides: {
				root: {
					color: restaurentsTheme.palette.primary.main,
				},
			},
		},
		MuiOutlinedInput: {
			styleOverrides: {
				notchedOutline: {
					borderColor: restaurentsTheme.palette.common.secondaryGreyText,
				},
			},
		},
		MuiBackdrop: {
		...
		MuiDialog: {
			styleOverrides: {
				paper: { width: `100%`, borderRadius: `12px`, boxShadow: `0px 12px 60px 0px #00000033` },
			},
		},
	};
};

export const useRestaurentsTheme = (): Theme => {
	const components = useThemeComponents();
	return { ...restaurentsTheme, components };
};
